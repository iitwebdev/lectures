...
import atexit
import socket
## FIXME: is logging too heavy?
import logging
import subprocess
import os
import shlex
from olpcserver.exactproxy import proxy_exact_request
from olpcserver.util import add_info_to_exception
...
class SubprocessApp(object):
    ...
    def __init__(self, start_script, cwd=None, environ=None, port=None,
        ...
        self.must_allocate_port = port is None
        self.port = port
    ...
    def spawn_subprocess(self):
        ...
        if self.port is None:
            self.allocate_port()
        script = script.replace('__PORT__', str(self.port))
        script = shlex.split(script)
        try:
            ## FIXME: should this do something with stdout/stderr?
            self.proc = subprocess.Popen(script, cwd=self.cwd,
                                         env=self.environ, shell=False)
        except Exception, e:
            print script
            add_info_to_exception(e, 'while running script %r' % script)
            raise
    ...
    def shutdown_monitor(self):
        while 1:
            now = time.time()
            print 'checking for idle at %s; last: %s; limit: %s' % (now, self.last_request, self.idle_shutdown)
            if self.proc is None:
                return
            ...
            print 'Trying again in %s seconds' % wait_time
    ...
    def close(self):
        # FIXME: should I acquire the lock here?
        if self.proc is not None:
            ...
            try:
                ...
                pass
            self.proc = None
            if self.must_allocate_port:
                # Reallocate the port next time
                self.port = None
